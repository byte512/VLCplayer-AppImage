...
LINUX_DEPLOY_QT_EXCLUDE_COPYRIGHTS=true appimage-wrapper linuxdeployqt-continuous-x86_64.AppImage vlc-$VERSION/build/org.videolan.vlc.desktop  -bundle-non-qt-libs -exclude-libs=libfreetype.so.6 -updateinformation="gh-releases-zsync|cmatomic|VLCplayer-AppImage|stable|VLC_media_player.*$VERSION*-x86_64.AppImage.zsync"
LINUX_DEPLOY_QT_EXCLUDE_COPYRIGHTS=true ARCH=x86_64 appimage-wrapper linuxdeployqt-continuous-x86_64.AppImage  vlc-$VERSION/build/org.videolan.vlc.desktop -updateinformation="gh-releases-zsync|cmatomic|VLCplayer-AppImage|stable|VLC_media_player.*$VERSION*-x86_64.AppImage.zsync" -appimage
mkdir -p release
