...
LINUX_DEPLOY_QT_EXCLUDE_COPYRIGHTS=true appimage-wrapper linuxdeployqt-continuous-x86_64.AppImage vlc-$VERSION/build/org.videolan.vlc.desktop  -bundle-non-qt-libs -exclude-libs=libfreetype.so.6 -updateinformation="gh-releases-zsync|cmatomic|VLCplayer-AppImage|continuous|VLC_media_player.*$VERSION*-x86_64.AppImage.zsync"
LINUX_DEPLOY_QT_EXCLUDE_COPYRIGHTS=true ARCH=x86_64 appimage-wrapper linuxdeployqt-continuous-x86_64.AppImage  vlc-$VERSION/build/org.videolan.vlc.desktop -appimage -updateinformation="gh-releases-zsync|cmatomic|VLCplayer-AppImage|continuous|VLC_media_player.*$VERSION*-x86_64.AppImage.zsync"
mkdir -p release
cp ./VLC_media_player*.zsync release/
cp ./VLC_media_player*.AppImage release/
md5sum ./VLC_media_player*.AppImage > release/VLC_media_player.*$VERSION*-x86_64.AppImage.md5.txt
